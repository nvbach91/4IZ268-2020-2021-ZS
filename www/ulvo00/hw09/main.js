...
const textInput = document.querySelector('#cipherText');
const shiftInput = document.querySelector('#shift');
const buttonInput = document.querySelector('#decipherButton');
buttonInput.addEventListener('click', () => {
    const cipherText = textInput.value;
    const usedKey = shiftInput.value;
    const result = caesarDecipher(cipherText, usedKey);
    const decipheredText = document.createElement('p');
    decipheredText.innerText = result;
    document.querySelector('#decipheredText').appendChild(decipheredText);
});
...
const shiftString = (str, shift) => {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        newStr = newStr + shiftChar(str[i], shift);
    }
    return newStr
};
