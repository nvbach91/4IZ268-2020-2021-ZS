$(document).ready(() => {
    ...
    const addRecipe = (r) => {
        ...
        const recipeImage = $(`<img class="w-1/3 md:w-1/5 lg:w-1/4 object-cover rounded-tl-md" src="${r.image}" alt="${r.title}">`);
        ...
        const recipeSummary = $(`<div class="py-4 px-8 hidden">
            <p class="summary text-sm">${r.summary}</p>
        </div>`);

        const recipeSummaryButtonContainer = $(`<div class="w-full bg-green-100 rounded-b-md">`);

        const recipeSummaryButton = $(`<button class="w-full py-1 text-center text-green-500">`);
        const recipeSummaryButtonIcon = $(`<i class="fas fa-chevron-down">`);

        recipeContainer.append(recipeImage, recipeInfoContainer);
        recipeLink.append(recipeScore, recipeContainer, recipeSummary);
        
        recipeSummaryButton.append(recipeSummaryButtonIcon);
        recipeSummaryButtonContainer.append(recipeSummaryButton);

        recipe.append(recipeLink, recipeSummaryButtonContainer);

        recipeSummaryButton.click((e) => {
            if(recipeSummary.hasClass('hidden')){
                recipeSummaryButtonIcon.removeClass('fa-chevron-down');
                recipeSummaryButtonIcon.addClass('fa-chevron-up');
                recipeSummary.removeClass('hidden');
            } else {
                recipeSummaryButtonIcon.removeClass('fa-chevron-up');
                recipeSummaryButtonIcon.addClass('fa-chevron-down');
                recipeSummary.addClass('hidden');
            }
        });
