...
const caesarDecipher = (cipherText, keyNumber) => {
    ...
    deciphered = shiftString(cipherText.toUpperCase(), keyNumber);
    return deciphered;
};
const CipherTextPlace = document.querySelector('#inputcipher');
const InputKeyPlace = document.querySelector('#inputkey');
const DecipherButton = document.querySelector('#buttondecipher');
const OutputPlace = document.querySelector('#output');
const Table = document.querySelector('#table');

DecipherButton.addEventListener('click', (e) => {
    let cipherText = CipherTextPlace.value;
    let usedKey = InputKeyPlace.value;
    if( InputKeyPlace.value > 26) {
        Table.insertBefore(impossible, DecipherButton);
    } 
    else {
        OutputPlace.innerText = caesarDecipher(cipherText, usedKey);
    }
    e.preventDefault();
});
